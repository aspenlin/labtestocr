...
class Request(object):
    ...
    def headers(self):
        ...
        for security in self.collection.api.__schema__.get('security', []):
            for key, header in list(self.collection.apikeys.items()):
                if key in security:
                    headers[header] = ''
        ...
        lines = [':'.join(line) for line in list(headers.items())]
        return '\n'.join(lines)
...
class PostmanCollectionV1(object):
    ...
    def requests(self):
        ...
        for path, operations in list(self.api.__schema__['paths'].items()):
    ...
    def apikeys(self):
        return dict(
            (name, secdef['name'])
            for name, secdef in list(self.api.__schema__.get('securityDefinitions').items())
